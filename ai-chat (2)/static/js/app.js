...
const proactive = {
    ...
    scheduleNext() {
        if (!this.enabled || !state.currentCharId) return;
    ...
    async tick() {
        ...
        const minutesElapsed = (Date.now() - this.lastActivity) / 60000;
        // 第1分钟为空白期，概率为0；第2分钟起基础3% + 每分钟递增a%
        ...
        if (Math.random() * 100 < probability) {
            var sent = false;
            try {
                var data = await api("POST", "/api/chat/" + state.currentCharId + "/proactive");
                appendMessage("assistant", data.reply);
                appendSystemMessage("💤 角色主动发起了对话");
                loadCharacters();
                sent = true;
            } catch (e) {
                console.error("主动消息失败:", e);
            }
            if (sent) {
                // 已发过消息，暂停定时器，等用户回复后由 reset() 重启
...
function syncProactiveUI() {
    ...
    if (!cid) {
        clearTimeout(proactive.timerId);
        proactive.timerId = null;
    ...
    var wasEnabled = localStorage.getItem("proactive_" + cid);
    if (wasEnabled) {
        proactive.enabled = true;
        proactive.lastActivity = Date.now();
        btn.textContent = "💤 开";
...
function toggleProactive() {
    console.log("toggleProactive called, cid=" + state.currentCharId + " enabled=" + proactive.enabled);
    ...
    var btn = document.getElementById("btnProactive");
    var bar = document.getElementById("proactiveBar");
    console.log("toggleProactive: btn=" + !!btn + " bar=" + !!bar);
    ...
    if (proactive.enabled) {
        ...
        proactive.timerId = null;
        proactive.enabled = false;
        localStorage.removeItem("proactive_" + state.currentCharId);
        if (btn) {
            btn.textContent = "💤 关";
            btn.style.borderColor = "";
            btn.style.color = "";
        ...
        proactive.enabled = true;
        proactive.lastActivity = Date.now();
        localStorage.setItem("proactive_" + state.currentCharId, "1");
        if (btn) {
            btn.textContent = "💤 开";
            btn.style.borderColor = "#10b981";
            btn.style.color = "#10b981";
...
function renderCharacterList() {
    ...
    dom.characterList.innerHTML = state.characters
        .map(
            (c) => `
        <div class="char-card${c.id === state.currentCharId ? " active" : ""}" data-id="${c.id}">
...
// saveSettings 已通过 HTML onclick 绑定

// 面板 Tab 切换（全局函数，供 HTML onclick 调用）
...
(async function init() {
    ...
    try {
        ...
        // 若无角色，轮询 API 直至加载成功（最多重试 8 次，间隔 1 秒）
